...
const tasks_1 = require("../lib/ai/tasks");
const context_1 = require("../lib/ai/context");
const checkpoints_1 = require("../lib/agent/checkpoints");
const PORT = parseInt(process.env.BE_PORT || '6655', 10);
// In-memory runtime state for backend API
let tasks = [...tasks_1.INITIAL_TASKS];
let checkpoints = [...checkpoints_1.INITIAL_CHECKPOINTS];
...
const server = node_http_1.default.createServer(async (req, res) => {
    ...
    if (pathname === '/api/agent/tasks/next' && method === 'GET') {
        ...
        if (inProgress) {
            const context = (0, context_1.buildAgentTaskContext)(inProgress, 'Aplikasi Booking Lapangan Futsal', 'Platform reservasi lapangan olahraga real-time.');
            return sendJson(res, 200, {
                success: true,
                hasTask: true,
                status: 'IN_PROGRESS',
                task: inProgress,
                context,
                markdownPrompt: (0, context_1.formatAgentMarkdownPrompt)(context),
        ...
        const nextTask = tasks.find((t) => {
            if (t.status !== 'TODO')
                return false;
            if (!t.depends_on_task_ids || t.depends_on_task_ids.length === 0)
                return true;
        ...
        const context = (0, context_1.buildAgentTaskContext)(nextTask, 'Aplikasi Booking Lapangan Futsal', 'Platform reservasi lapangan olahraga real-time.');
        return sendJson(res, 200, {
            success: true,
            hasTask: true,
            status: 'TODO',
    ...
    if (contextMatch && method === 'GET') {
        ...
        const context = (0, context_1.buildAgentTaskContext)(task, 'Aplikasi Booking Lapangan Futsal', 'Platform reservasi lapangan olahraga real-time dengan hold slot 15 menit dan QRIS.');
        return sendJson(res, 200, {
            success: true,
            taskId,
            context,
