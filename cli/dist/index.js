...
async function runCli(args) {
    ...
    switch (command) {
        case 'login': {
            ...
            const res = await (0, api_client_1.requestApi)('/api/agent/auth', {
                method: 'POST',
                token,
            });
            if (!res.success) {
        ...
        case 'status': {
            ...
            const authRes = await (0, api_client_1.requestApi)('/api/agent/auth', { method: 'POST' });
        ...
        case 'next': {
            ...
            const res = await (0, api_client_1.requestApi)('/api/agent/tasks/next');
            if (!res.success) {
            ...
            const task = res.task;
            (0, config_1.saveConfig)({ activeTaskId: task.id });
            console.log(`\n┌─────────────────────────────────────────────────────────────┐`);
            ...
            if (task.status !== 'IN_PROGRESS') {
                ...
                if (confirm.toLowerCase() === 'y') {
                    const startRes = await (0, api_client_1.requestApi)(`/api/agent/tasks/${task.id}/start`, { method: 'POST' });
                    if (startRes.success) {
        ...
        case 'context': {
            ...
            const res = await (0, api_client_1.requestApi)(`/api/agent/tasks/${taskId}/context`);
            if (!res.success) {
        ...
        case 'done': {
            ...
            const res = await (0, api_client_1.requestApi)(`/api/agent/tasks/${taskId}/complete`, { method: 'POST' });
            if (!res.success) {
